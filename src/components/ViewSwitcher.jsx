...
const ViewSwitcher = ({ view, onViewChange, title, onNavigate, onToday }) => (
  <>
    <div className="view-tabs">
      <button className={`tab ${view === 'daily' ? 'active' : ''}`} onClick={() => onViewChange('daily')}>
        <Calendar size={16} /> Daily
      </button>
      <button className={`tab ${view === 'weekly' ? 'active' : ''}`} onClick={() => onViewChange('weekly')}>
        <List size={16} /> Weekly
      </button>
      <button className={`tab ${view === 'monthly' ? 'active' : ''}`} onClick={() => onViewChange('monthly')}>
